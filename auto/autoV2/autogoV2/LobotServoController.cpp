...
void LobotServoController::moveServos(LobotServo servos[], uint8_t Num, uint16_t Time)
{
	...
	Serial1.write(buf, buf[2] + 2); //发送帧，长度为数据长度+两个字节的帧头
}
...
void LobotServoController::receiveHandle()
{
	...
	case 2:
		...
		break;
	case 3:
		if (Serial1.readBytes(buf, len - 1) > 0)
			getHeader = 4;
		else{
			len = 0;
			getHeader = 0;
